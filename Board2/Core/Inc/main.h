/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32g4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define Motor3_Pin GPIO_PIN_2
#define Motor3_GPIO_Port GPIOC
#define Motor4_Pin GPIO_PIN_3
#define Motor4_GPIO_Port GPIOC
#define SonarLeftEcho_Pin GPIO_PIN_10
#define SonarLeftEcho_GPIO_Port GPIOB
#define SonarRightEcho_Pin GPIO_PIN_11
#define SonarRightEcho_GPIO_Port GPIOB
#define SonarRightTrig_Pin GPIO_PIN_12
#define SonarRightTrig_GPIO_Port GPIOB
#define Motor1_Pin GPIO_PIN_8
#define Motor1_GPIO_Port GPIOA
#define Motor2_Pin GPIO_PIN_9
#define Motor2_GPIO_Port GPIOA
#define SonarLeftTrig_Pin GPIO_PIN_10
#define SonarLeftTrig_GPIO_Port GPIOA
#define RTR_IN_Pin GPIO_PIN_10
#define RTR_IN_GPIO_Port GPIOC
#define RTR_OUT_Pin GPIO_PIN_11
#define RTR_OUT_GPIO_Port GPIOC
#define SonarFrontEcho_Pin GPIO_PIN_3
#define SonarFrontEcho_GPIO_Port GPIOB
#define SonarFrontTrig_Pin GPIO_PIN_6
#define SonarFrontTrig_GPIO_Port GPIOB
#define LedDebug_Pin GPIO_PIN_7
#define LedDebug_GPIO_Port GPIOB

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
